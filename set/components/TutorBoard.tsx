...
import { Maximize2, Minimize2, GraduationCap, ChevronRight, Play, Check, BookOpen, Loader2, Search, Library, AlertCircle, RefreshCw } from 'lucide-react';
      ...
      <div className="h-full flex flex-col bg-[#1a1c1e] border-8 border-[#3D2B1F] rounded-lg relative overflow-hidden shadow-2xl">
         ...
         <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-[radial-gradient(circle_at_center,rgba(40,40,40,1)_0%,rgba(20,20,20,1)_100%)]">
             {isLoading ? (
                 ...
                 <div className="max-w-md bg-black/40 border border-red-500/30 p-8 rounded-3xl backdrop-blur-xl">
                    <div className="bg-red-500/20 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6">
                        <AlertCircle className="text-red-500" size={32} />
                    </div>
                    <h3 className="text-white font-mono text-xl font-black uppercase mb-2">Neural Link Interrupted</h3>
                    <p className="text-slate-400 text-sm mb-8 leading-relaxed">
                        {error.includes("quota") ? "API Quota Limit Reached. Please wait a minute or check your plan details." : "Unable to synchronize with the Apex core. The signal is weak."}
                    </p>
                    <button 
                        onClick={() => onStartLecture(topicInput || "Technical Analysis")}
                        className="bg-red-500 text-black px-10 py-4 rounded-xl font-black text-xs uppercase tracking-[0.2em] flex items-center gap-2 mx-auto hover:bg-red-400 active:scale-95 transition-all"
                    >
                        RETRY LINK <RefreshCw size={16} />
                    </button>
                 </div>
             ) : (
                    ...
                    <form onSubmit={handleStartSubmit} className="relative group mb-12">
                        ...
                        <div className="relative flex">
                            ...
                            <input 
                                ...
                                placeholder="Search Strategy, Book, or Great..."
                                className="w-full bg-black border-2 border-slate-700 p-6 pl-16 rounded-2xl text-white text-base focus:outline-none focus:border-emerald-500 transition-all font-mono"
                            />
                            <button 
                                type="submit"
